...
import type { ContentQualityResult } from './contentQuality.js';

const MODEL = 'gemini-2.5-flash';

/** Website context: only cleaned extracted content + metadata. No raw URL for analysis. */
export interface WebsiteExtraction {
  /** Plain text extracted from the page (HTML stripped). */
...
/** API/Server probe (snippets only; we do not send raw URLs to Gemini). */
...
export interface AnalyzeInput {
  businessName: string;
  businessType?: string;
  /** When present, only extracted content + metadata are sent; never the URL. */
  website: WebsiteExtraction | null;
  serverProbe: ServerProbeInput | null;
}
...
export function buildAnalysisPrompt(input: AnalyzeInput): string {
  ...
  if (businessType) context += ` Type of business: ${businessType}.`;
 ...
 */
export function safeParseAnalysisResponse(
  text: string
): { tasks: unknown[]; recommendations: string[] } | null {
  ...
  try {
    const parsed = JSON.parse(jsonStr) as { tasks?: unknown[]; recommendations?: unknown };
    if (!parsed || !Array.isArray(parsed.tasks) || !Array.isArray(parsed.recommendations)) return null;
    ...
    if (start >= 0 && end > start) {
      try {
        const parsed = JSON.parse(trimmed.slice(start, end)) as { tasks?: unknown[]; recommendations?: unknown };
...
function toTask(
  raw: { title: string; priority?: string; dueDate?: string; category?: string },
  index: number
...
export async function analyzeWithGemini(
  ...
  const tasks = parsed.tasks
    .filter(
      (t): t is Record<string, unknown> =>
        ...
        String((t as Record<string, unknown>).title).length > 0
    )
    .slice(0, 20)
    .map((t, i) =>
      toTask(
        {
          ...
          dueDate: (t.dueDate as string) ?? '2026-03-31',
  ...
  const extractionMetadata: ExtractionMetadata = input.website
