...
interface ZapierStatePayload {
  userId: string;
  nonce: string;
  purpose: 'zapier_connect';
}

function buildZapierState(userId: string): string {
  const nonce = crypto.randomBytes(16).toString('hex');
  const payload: ZapierStatePayload = { userId, nonce, purpose: 'zapier_connect' };
  return jwt.sign(payload, JWT_SECRET, { expiresIn: '600s' }); // 10 min
}

function verifyZapierState(state: string): ZapierStatePayload | null {
  try {
    const decoded = jwt.verify(state, JWT_SECRET) as ZapierStatePayload;
    if (decoded.purpose !== 'zapier_connect' || !decoded.userId) return null;
    return decoded;
  } catch {
    return null;
  }
...
function buildZapierConnectUrl(userId: string): string {
...
zapierRouter.get('/status', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  if (!isDbConnected()) {
...
zapierRouter.get('/connect', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  if (!isDbConnected()) {
    res.status(503).json({ error: 'Database not available' });
    return;
  }
...
zapierRouter.get('/connect-url', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  if (!isDbConnected()) {
    res.status(503).json({ error: 'Database not available' });
    return;
  }
...
zapierRouter.get('/callback', async (req: Request, res: Response): Promise<void> => {
  ...
  const errorParam = (req.query.error as string)?.trim();

  if (errorParam) {
    const errorDesc = (req.query.error_description as string)?.trim();
  ...
  try {
    ...
    await ZapierConnection.updateMany(
      { userId: statePayload.userId, provider: 'zapier' },
      { status: 'revoked' }
    );
...
zapierRouter.post('/disconnect', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  if (!isDbConnected()) {
    res.status(503).json({ error: 'Database not available' });
    return;
  }
