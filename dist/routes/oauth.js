...
const DEFAULT_REDIRECT_URI = 'https://zapier.com/dashboard/auth/oauth/return/App237819CLIAPI/';
function getOAuthClientId() {
    return (process.env.ZAPIER_OAUTH_CLIENT_ID ||
        process.env.ZAPIER_CLIENT_ID ||
        '').trim();
}
function getOAuthClientSecret() {
    return (process.env.ZAPIER_OAUTH_CLIENT_SECRET ||
        process.env.ZAPIER_CLIENT_SECRET ||
        '').trim();
}
function getAllowedRedirectUris() {
    const csvUris = (process.env.ZAPIER_OAUTH_REDIRECT_URIS ?? '').trim();
    const singleUri = (process.env.ZAPIER_REDIRECT_URL ?? '').trim();
    const defaults = [DEFAULT_REDIRECT_URI];
    const parsed = [
        ...csvUris.split(','),
        ...(singleUri ? [singleUri] : []),
        ...defaults,
...
function normalizeRedirectUriForComparison(uri) {
    try {
...
function normalizeAuthorizeParams(input) {
    return {
...
function validateAuthorizeParams(params) {
    if (!getOAuthClientId() || !getOAuthClientSecret()) {
        return 'OAuth is not configured. Set ZAPIER_OAUTH_CLIENT_ID/ZAPIER_OAUTH_CLIENT_SECRET (or legacy ZAPIER_CLIENT_ID/ZAPIER_CLIENT_SECRET).';
    }
    ...
    if (params.clientId !== getOAuthClientId()) {
        return 'Invalid client_id. Confirm Zapier OAuth client credentials match backend env vars.';
    }
    const normalizedIncomingRedirect = normalizeRedirectUriForComparison(params.redirectUri);
    const allowed = getAllowedRedirectUris().some((uri) => normalizeRedirectUriForComparison(uri) === normalizedIncomingRedirect);
    if (!allowed) {
        return 'Invalid redirect_uri.';
    }
    return null;
}
function buildOAuthErrorRedirect(redirectUri, state, error, errorDescription) {
    const location = new URL(redirectUri);
    location.searchParams.set('error', error);
    location.searchParams.set('state', state);
    if (errorDescription)
        location.searchParams.set('error_description', errorDescription);
    return location.toString();
}
function renderAuthorizePage(params, message) {
    const defaultEmail = (process.env.ZAPIER_OAUTH_DEFAULT_EMAIL || process.env.ZAPIER_TEST_EMAIL || '').trim();
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
function createAuthorizationCode(params, identity) {
    return jwt.sign({
    ...
    }, JWT_SECRET, { expiresIn: getCodeTtlSeconds() });
}
function createAccessToken(claims) {
    return jwt.sign({
        ...
        email: claims.email,
        scope: claims.scope,
    }, JWT_SECRET, { expiresIn: getAccessTtlSeconds() });
}
function createRefreshToken(claims) {
    return jwt.sign({
        ...
        scope: claims.scope,
        jti: crypto.randomBytes(12).toString('hex'),
    }, JWT_SECRET, { expiresIn: getRefreshTtlSeconds() });
}
...
oauthRouter.get('/authorize', (req, res) => {
    const params = normalizeAuthorizeParams(req.query);
    const error = validateAuthorizeParams(params);
    if (error) {
        if (params.redirectUri && params.state) {
            res.redirect(302, buildOAuthErrorRedirect(params.redirectUri, params.state, 'invalid_request', error));
            return;
        }
...
oauthRouter.post('/token', async (req, res) => {
    ...
    if (grantType === 'authorization_code') {
        ...
        try {
            ...
            const normalizedCodeRedirect = normalizeRedirectUriForComparison(decoded.redirect_uri ?? '');
            const normalizedRequestRedirect = normalizeRedirectUriForComparison(redirectUri);
            if (decoded.client_id !== clientId || normalizedCodeRedirect !== normalizedRequestRedirect) {
                res.status(400).json({
                    error: 'invalid_grant',
            ...
            if (isDbConnected()) {
                ...
                await ZapierConnection.updateMany({ userId: decoded.sub, provider: 'zapier' }, { status: 'revoked' });
    ...
    if (grantType === 'refresh_token') {
        ...
        try {
            ...
            if (decoded.token_type !== 'oauth_refresh' ||
            ...
            if (isDbConnected()) {
                const expiresAt = new Date(Date.now() + tokens.expires_in * 1000);
                await ZapierConnection.findOneAndUpdate({ userId: decoded.sub, provider: 'zapier', status: 'active' }, {
                    accessToken: tokens.access_token,
                    refreshToken: tokens.refresh_token,
                    expiresAt,
                    scope: decoded.scope ?? '',
