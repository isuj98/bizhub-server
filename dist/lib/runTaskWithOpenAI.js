...
function buildPrompt(taskTitle, websiteContext) {
...
function parseJsonFromResponse(text) {
    ...
    try {
        ...
        if (!parsed || typeof parsed.summary !== 'string' || typeof parsed.outcome !== 'string')
            return null;
        const suggestedContent = Array.isArray(parsed.suggestedContent)
            ? parsed.suggestedContent.filter((s) => typeof s === 'string')
            : [];
        return {
...
export async function runTaskWithOpenAI(apiKey, taskTitle, websiteContext) {
